...
class ClaimLinearProbingDetector(ProbingDetectorBase):
    """
    Linear probing detector for claim-level classification.

    Architecture: Uses linear classifier on pooled hidden states for each claim split.
    Differs from sequence-level by splitting responses into claim segments first.
    """
    ...
    def _load_model(self, load_dir: Path) -> None:
        ...
        # Create model with loaded config (match setup_model() so architecture matches)
