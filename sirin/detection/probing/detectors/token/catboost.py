...
class TokenCatboostProbingDetector(ProbingDetectorBase):
    ...
    def detect(
        ...
        **kwargs,
        ...
        # Check for NaN values and empty answer indices
        ...
        token_probs, token_preds = handle_binary_multiclass_probs(
            token_probs, self.threshold, is_binary=(self.config.num_classification_heads <= 2)
        )

        # Rearrange token predictions to character-level per sample
        char_probs, char_preds = rearrange_token_predictions(token_probs, token_preds, offsets, all_lengths)
