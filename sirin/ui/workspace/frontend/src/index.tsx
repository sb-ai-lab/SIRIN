...
function scoreLabel(semantics?: string, scaleLabel?: string | null): string | null {
  ...
  if (["calibrated_probability", "calibratedProbability"].includes(semantics ?? "")) return "probability"
  if (["relative_within_answer", "relativeWithinAnswer"].includes(semantics ?? "")) return "relative score"
  if (["thresholded_raw_score", "thresholdedRawScore"].includes(semantics ?? "")) return "raw score"
  ...
  if (semantics === "verdict") return "verdict score"
  return null
}
  ...
  return <>
    <header className="shell-header">
      ...
      <nav className="workspace-tabs" aria-label="Workspace">
        {(["analyze", "runs", "diagnostics"] as WorkspaceName[]).map((item) => <button type="button" key={item} data-workspace-tab={item} className={workspace === item ? "active" : ""} aria-current={workspace === item ? "page" : undefined} onClick={() => onWorkspace(item)}><Icon name={item} />{titleCase(item)}</button>)}
      </nav>
    ...
    <section className="page-head">
      ...
      <h1>{title ?? (workspace === "analyze" ? "See where an answer leaves the evidence." : workspace === "runs" ? "Every result, with its receipts." : "Know what SIRIN is running.")}</h1>
      <p>{subtitle ?? (workspace === "analyze" ? "Generate or supply an answer. SIRIN checks it against context and makes uncertainty legible." : workspace === "runs" ? "Review immutable outcomes and carry portable records between sessions." : "Inspect the active runtime without exposing sensitive internals.")}</p>
    </section>
  </>
}
...
function ResultCard({ run, motion, onAction, onPrepareRerun }: { run: RunRecord; motion: MotionName; onAction: (type: string, payload: Record<string, unknown>) => void; onPrepareRerun: (run: RunRecord) => void }) {
  ...
  return <section className={`result-card ${riskClass(verdictValue ?? verdict)}${reveal ? " is-reveal" : ""}`} style={reveal ? ({ "--reveal-total": `${revealTotalMs}ms` } as CSSProperties) : undefined} aria-live="polite">
    <div className="result-heading">
      ...
      {!isSpanResult && finite(score) !== null && metricLabel && <div className="score-orb"><strong>{scoreText(score, semantics)}</strong><span>{metricLabel}</span></div>}
    </div>
...
function AnalyzeWorkspace({ payload, draft, setDraft, busy, motion, onAction, onPrepareRerun }: { payload: WorkspacePayload; draft: WorkspaceAnalyzeDraft; setDraft: (draft: WorkspaceAnalyzeDraft, checkpoint?: boolean) => void; busy: boolean; motion: MotionName; onAction: (type: string, payload: Record<string, unknown>) => void; onPrepareRerun: (run: RunRecord) => void }) {
...
function WorkspaceApp({ componentKey, payload, setStateValue, setTriggerValue }: { componentKey: string; payload: WorkspacePayload; setStateValue: (name: keyof WorkspaceFrontendState, value: WorkspaceFrontendState[keyof WorkspaceFrontendState]) => void; setTriggerValue: (name: keyof WorkspaceFrontendState, value: WorkspaceFrontendState[keyof WorkspaceFrontendState]) => void }) {
  ...
  return <div ref={workspaceRootRef} className="sirin-workspace" data-theme={appearance.theme} data-motion={appearance.motion}>
    <div className="shell">
      ...
      {workspace === "analyze" && <AnalyzeWorkspace payload={payload} draft={draft.analyze} setDraft={setAnalyzeDraft} busy={busy} motion={appearance.motion} onAction={emit} onPrepareRerun={prepareRerun} />}
