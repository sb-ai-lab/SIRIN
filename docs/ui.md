...
Curated examples fill the editor but do not run automatically. A recorded replay uses the exact verified stored answer and messages, then runs detection live. It is labelled **Recorded answer · live detection** and is distinct from **Generated now**, **Supplied answer · live detection**, and **Imported snapshot**. SHA-256 verification establishes artifact integrity, not answer truthfulness. Scoring/generating is always the pink primary action on the right; when a curated example is selected, replaying its verified answer appears beside it as the secondary action.
