...
`SIRIN_UI_HOSTED=1` turns the app into the GPU-free paper demo (the Hugging Face Space profile): the API judge presets run live (Span default, plus Sequence, Answerability, Claim, and Sequence with verbalized confidence — the last one asks the judge to state its own confidence, so routes without logprobs still score), and every other preset (both PsiloQA probes, the three uncertainty presets, the LongMemEval Sequence TabPFN) is selectable as replay-only: the landing seeds one **Recorded result** card per preset and Replay serves that preset's verified recording — never live scoring, which needs weights the Space does not ship.

For a replay-only preset the Analyze editor renders that preset's recorded case read-only, disables **Generate & score** (live scoring is unavailable), and offers **Replay recorded answer** as the sole live action; Replay always resolves the active preset's own recording, so it works whichever example is loaded. Because the free OpenRouter route often returns no reply logprobs — leaving the class-token sequence and answerability judges with an honest verdict but no probability — hosted orders the verbalized-confidence sequence judge ahead of the plain one and nudges toward it from any scoreless hallucination-judge card; the Span judge (spans + k/n agreement) needs no logprobs and stays the default.

Backends are API-only (OpenRouter first), device is forced to `cpu`, and the landing keeps the Qwen3.5-4B probe card as the hero. Live detection runs through the visitor's pasted key; recorded cards render with no key at all. `SIRIN_OPENROUTER_MODEL` sets the pre-filled judge model (the Space uses a free OpenRouter route). Space packaging and deploy tooling live in a separate repo (`sirin_deploy`): its `make_space.py` stages and self-checks the tree from this checkout, and its `hf-space/` holds the Dockerfile, CPU requirements, and Space card.
...
- Trusted-local Custom judge endpoints (`SIRIN_CUSTOM_OPENAI_BASE_URL`, defaulting to the local vLLM at `http://localhost:8000/v1`) get thinking disabled by default for verdict/annotation stability; set `SIRIN_CUSTOM_JUDGE_THINKING=1` to re-enable it. When the endpoint is reachable, its `/models` list populates the Judge model choices (probed once per session).
